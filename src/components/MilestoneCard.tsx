...
const MilestoneCard = ({ milestone, isEven, isLast }: MilestoneCardProps) => {
  ...
  return (
    <div className={`flex flex-col ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'} items-center mb-24 relative`}>
      ...
      <div className={`w-full md:w-5/12 ${isEven ? 'md:pr-12' : 'md:pl-12'} text-center md:text-left ${isEven ? 'md:text-right' : ''}`}>
        <div 
          className={`bg-white p-6 rounded-lg shadow-lg border-2 border-primary/20 mb-4 transform transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}
        >
      ...
      <div className={`w-full md:w-5/12 ${isEven ? 'md:pl-12' : 'md:pr-12'} mt-8 md:mt-0`}>
        <div className="relative bg-secondary border-8 border-white shadow-lg rounded-md overflow-hidden hover:shadow-2xl transition-shadow duration-300" style={{ paddingBottom: '150%' }}>
          {image ? (
            <img 
              src={image} 
              alt={`Foto de ${milestone.title}`} 
              className="absolute inset-0 w-full h-full object-cover"
            />
          ) : (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-accent/20 p-4">
